...
class crossword implements Icrossword{
    ...
    async init(){
        let board;
        let year = Math.floor(Math.random() * (2017 - 1976 + 1)) + 1976;
        let month = String(Math.floor(Math.random() * (12 - 1 + 1)) + 1);
        if(parseInt(month) < 10) month = '0' + month
        let day = String(Math.floor(Math.random() * (31 - 1 + 1)) + 1);
        if(parseInt(day) < 10) day = '0' + day
        let req = await fetch(`https://raw.githubusercontent.com/doshea/nyt_crosswords/master/${year}/${month}/${day}.json`);
    ...
    async genimage(){
        ...
        async function drawGridCell(x: number, y: number, bgColor: number, letter: string = null, index = 0) {
            ...
            for (let i = 0; i < gridRows; i++) {
                for (let j = 0; j < gridSize; j++) {
                    ...
                    if (game.grid[a] == '.') {
                        ...
                        if(game.usergrid[a].toUpperCase() == game.grid[a]){
                            await drawGridCell(x, y, 0x538D4EFF, game.usergrid[a], game.gridnums[a]);
                        }
                        else await drawGridCell(x, y, 0xffffffff, game.usergrid[a], game.gridnums[a]);
                    }
                    a++;
                }
            }
...
export async function run(client: pkg.Client, msg: pkg.Message, args: string[]) {
    ...
    const chat = await msg.getChat();
    if(chats.includes(chat.id._serialized)) return msg.reply('A game is active in this chat!')
